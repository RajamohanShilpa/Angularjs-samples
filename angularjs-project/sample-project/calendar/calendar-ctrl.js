(function() {
    angular.module('angularProject.sampleProject.calendarApp', ['ngGrid', 'ui.calendar', 'ui.bootstrap'])
        .controller('calendarCtrl', function($scope, $http, $filter) {
            ...
            $scope.getData = function() {
                $http.get('/json-datas/listData.json').then(function(response) {
                    ...
                    //Full calendar version 2.0
                    $("div#calendar").fullCalendar('gotoDate', moment($scope.test[0].date));
            ...
            $scope.getDropDown = function() {
                $http.get('/json-datas/dropdownData.json').then(function(response) {
                    ...
                    _.each($scope.test, function(item) {$scope.colorAssign(item);});
            ...
            $scope.uiConfig = {
                calendar: {
                    ...
                    //complete:function(){
                    //    debugger;
                    //    if(typeof($scope.test) == 'object' && $scope.test !=undefined && $scope.test.length != 0)
                    //    {
                    //        _.each($scope.test, function(item) {$scope.colorAssign(item);});
                    //    }
                    //}
